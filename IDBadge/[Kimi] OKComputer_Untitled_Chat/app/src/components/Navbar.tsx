          ...
          {NAV_LINKS.map((link) => {
            ...
            return (
              <a
                ...
                style={{
                  color: isActive ? '#C8962E' : 'rgba(255,255,255,0.7)',
                }}
                onMouseEnter={(e) => {
                  if (!isActive) (e.target as HTMLElement).style.color = 'rgba(255,255,255,1)';
                }}
                onMouseLeave={(e) => {
                  if (!isActive) (e.target as HTMLElement).style.color = 'rgba(255,255,255,0.7)';
                }}
              >
                {link.label}
                {isActive && (
                  <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#C8962E]" />
                )}
              </a>
            );
          })}
